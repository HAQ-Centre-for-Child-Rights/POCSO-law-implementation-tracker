...
select state_name, district_name, est_code, court_name, cino, fil_no,date_of_filing, reg_no, reg_year_date, type_name, disp_nature, disp_name from case_details where fil_year = '0' and case_flag = '1'
```
...
state_list <- dbxSelect(con, 'select distinct state_name from case_details') %>% unlist(use.names = FALSE)

fetch_state_cases <- function(state_name){
  state_cases <- dbxSelect(con, glue("select * from case_details where state_name = '{state_name}'"))
