...
```{sql connection=con}
select * from case_details where cino = 'DLET010004652012' limit 5
```

```{sql connection=con}
select * from act where id = '3931'
```
...
acts_issue <- read_csv("datasets/to_standardise/acts_to_standardise.csv")
act_ids <- noquote(paste0(acts_issue$act_id[acts_issue$act_issue_flag == 1], collapse = ','))
```
...
```{r}
get_case_details <- glue("select cino, judgment_link, judgment_path from case_details where cino IN ({case_list})")
...
all_acts_with_issues <- all_cases_with_issues %>% group_by(act) %>% summarise(total_cases = length(cino), total_judgements = length(judgment_link[judgment_link != '']),
...
get_combined_acts <- all_acts_with_issues[grepl(pattern = '&',x = all_acts_with_issues$act_name_std),]
